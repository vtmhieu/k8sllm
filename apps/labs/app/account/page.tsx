  ...
  return (
    <main className="mx-auto grid w-[min(1160px,calc(100%-32px))] gap-8 py-12">
      <header>
        ...
        <h1 className="mt-4 max-w-4xl text-6xl font-black leading-[0.9] tracking-tight text-slate-950">
      ...
      <section className="grid gap-px border border-blue-100 bg-blue-100 md:grid-cols-3">
        <AccountPhase
          title="Free"
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          href="/challenges"
          className="min-h-12 border border-[#326ce5] bg-[#326ce5] px-5 py-3 font-black text-white transition hover:bg-blue-700"
        >
          Browse free challenges
        </Link>
        <a
          href="https://www.k8sllm.online/docs/labs"
          className="min-h-12 border border-blue-200 bg-white px-5 py-3 font-black text-slate-800 transition hover:border-[#326ce5] hover:bg-blue-50 hover:text-[#326ce5]"
        >
          Read public docs
        </a>
      </div>
...
function AccountPhase({ title, body }: { title: string; body: string }) {
  return (
    <article className="bg-white/90 p-5">
      <h2 className="m-0 font-mono text-3xl font-black text-slate-950">{title}</h2>
      <p className="m-0 mt-3 text-sm leading-relaxed text-slate-600">{body}</p>
    </article>
  );
}
